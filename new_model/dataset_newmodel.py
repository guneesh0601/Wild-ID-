...
class WildIDStreamDataset:
    # Added 'augment=False' parameter
    def __init__(self, hf_dataset, num_classes=10127, augment=False):
        ...
        # Masking parameters from your screenshot
        self.freq_mask = T.FrequencyMasking(freq_mask_param=35)
        self.time_mask = T.TimeMasking(time_mask_param=85)
    ...
    def __iter__(self):
        ...
        while True:
            try:
                ...
                # Safe Log & Standardize
                mel_spec = torch.log(mel_spec + 1e-6)
                mel_spec = (mel_spec - mel_spec.mean()) / (mel_spec.std() + 1e-6)
                mel_spec = torch.clamp(mel_spec, -10.0, 10.0)

                # Augment Check (ONLY if self.augment is True)
